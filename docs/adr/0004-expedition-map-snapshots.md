...
This preserves historical context and makes later Map changes comparable to an Expedition's snapshot without a mandatory change note, a ranking artifact, or an Iteration lifecycle. The comparison distinguishes post-confirmation personal Scouting notes from newly charted Islands and shows evaluation value movement explicitly. If the current Map values newly make an unselected Island stronger than a selected Island, it presents a read-only notice only; it neither changes nor replaces the Expedition.
