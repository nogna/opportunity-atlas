...
The selected UI direction presents this journey with a compact rail at the left: set the lens, reveal and choose Islands, then complete Charter details. The lens step uses a focused planning board and a quiet compass visual to orient the team without recommending an Island or weight. It offers common planning-horizon choices plus an explicit free-text horizon. The rail makes the flow legible without turning the vertically scrolling content into tabs or blocking exploration.
