...
4. The team enters **Expedition planning** in two parts. First, it sets the horizon and team-controlled lens without seeing Island rankings, then explicitly reveals the resulting live ranking when ready. This keeps weighting choices independent of the current scores. Second, the team accepts the top Island or adds more before creating or reviewing a compact Island Charter for each selected Island. It may surface the selected Islands' prepared hypotheses and gaps, but does not ask the team to restate them. The confirmed Expedition presents a summary of those Charters rather than requiring a generic mission.

The selected UI direction presents this journey with a compact two-view rail at the left: planning workspace, then Charter details. Setting the lens and revealing or hiding its ranked shortlist are one reversible planning workspace; the shortlist itself shows each Island's add/remove state, so no separate selected-Islands summary is needed. Charter details remains a distinct next view and can return to the planning workspace. The lens uses a focused planning board and a quiet, sticky compass visual at the right to orient the team without recommending an Island or weight. It records a concrete planning window (number plus unit) and optionally the deadline or event that bounds it. The rail makes the flow legible without turning the vertically scrolling content into tabs or blocking exploration.
...
The planning horizon and lens are edited in the lens stage only; a persistent lens summary is retired because it competes with the journey. Islands chosen from the shortlist are described plainly as **Added to this Expedition**, not “Islands aboard.”
