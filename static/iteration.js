...
function planningRankNotice(ranked, selectedIds) {
  const highestSelected = ranked.find(island => selectedIds.includes(island.id));
  if (!highestSelected) return '';
  const selectedPosition = ranked.findIndex(island => island.id === highestSelected.id);
  const higherUnselected = ranked.slice(0, selectedPosition).find(island => !selectedIds.includes(island.id));
  if (!higherUnselected) return '';
  return `<aside class="planning-rank-notice"><p class="eyebrow">RANKING CHANGED</p><p><b>${escapeHtml(higherUnselected.title)}</b> now ranks above your selected <b>${escapeHtml(highestSelected.title)}</b>. Your Expedition draft has not changed.</p></aside>`;
}
...
function renderExpedition(payload) {
  ...
  document.querySelectorAll('[data-lens-weight]').forEach(input => input.addEventListener('input', () => {
    ...
    if (expeditionDraft.__rankingsRevealed) render(workspace);
